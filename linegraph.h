...
inline void drawLineGraph(int x[],int y[],int n)
{
    ...
    outtextxy(400, 30, (char*)"LINE GRAPH");
    ...
    setcolor(BLACK);
    for(int i=0;i<=LINE_TICKS;i++)
    {
        int val=(maxX*i)/LINE_TICKS;
        int px=LEFT+val*xs;
    ...
    for(int i=0;i<n;i++)
    {
        ...
        if(i>0)
        {
            int px2=LEFT+x[i-1]*xs;
            int py2=BOTTOM-y[i-1]*ys;

            bresenhamLineAnimated(px2,py2,px,py,BLUE,3);
        }

        bresenhamCircleAnimated(px,py,5,RED,5);
